...
class VisionSolver:
        ...
        if size == 3:
                ...
                for col in range(size):
                    if (row, col) not in cells_covered:
                        ...
                        if (row >= size - 2 and col >= size - 2) or random.random() < 0.3:
                            ...
                            cages.append(Cage("=", target, cage_cells))
                            continue
            ...
            if cells_covered != all_cells:
                ...
                for cell in missing_cells:
                    cages.append(Cage("=", random.randint(1, size), [cell]))

        # Create and return the puzzle
        return Puzzle(size, cages)

    def detect_puzzle(self):
        """Detect a KenKen puzzle from webcam or image"""
        try:
            # Get the image frame
            ...
            for contour in contours:
                ...
                if area > 5000:  # Minimum area threshold
                    ...
                    if len(approx) == 4:  # Looking for quadrilateral
                        ...
                        if 0.8 <= aspect_ratio <= 1.2 and area > max_area:  # Nearly square
    ...
    def _create_puzzle_from_vision(self, cages, warped):
        ...
        if not cages or warped is None:
            print("No valid cages or warped image detected")
            return None

        cell_size = warped.shape[0] // self.size
        processed_cages = []
        
        for cage in cages:
            # Get the top-left cell of the cage
            top_left = min(cage.cells)
            row, col = top_left
            
            # Extract the region containing the target and operation
            x = col * cell_size
            y = row * cell_size
            cell_img = warped[y:y+cell_size, x:x+cell_size]
            
            # Convert to grayscale if needed
            if len(cell_img.shape) == 3:
                cell_img = cv2.cvtColor(cell_img, cv2.COLOR_BGR2GRAY)
            
            # Split cell into regions for number and operation
            ...
            if operation == "=" and len(cage.cells) != 1:
                print(f"Warning: '=' operation cage at {top_left} has {len(cage.cells)} cells (should be 1)")
                continue
            
            if operation in ("-", "/") and len(cage.cells) != 2:
    ...
    def _detect_operation(self, cell_img, cage_size):
        ...
        if aspect_ratio > 2.5:  # Vertical line -> division
            ...
            return "-"
        elif solidity > 0.8:  # Very solid -> equals
            return "="
    ...
    def _update_display(self, original, processed, warped, edges):
        ...
        if edges is not None:
            self.ax[1, 1].imshow(edges, cmap='gray')
            self.ax[1, 1].set_title('Grid Edges')
        else:
            self.ax[1, 1].text(0.5, 0.5, "No edges detected", 
                            ha='center', va='center', fontsize=12)
            self.ax[1, 1].set_title('Grid Edges')
